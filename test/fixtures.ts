...
import GSNToken from '../build/GSNToken.json'
...
export async function governanceFixture(
  ...
  const gsnToken = await deployContract(wallet, GSNToken, [wallet.address, timelockAddress, now + 60 * 60, trustedForwarder])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, gsnToken.address, trustedForwarder])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { gsnToken, timelock, governorAlpha }
}
